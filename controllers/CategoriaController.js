...
function registrar(req, res) {
    ...
    categoria.save((err, categoria_save) => {
        if (err) {
            ...
            if (categoria_save) {
                res.status(200).send({ categoria_save });
            } else {
    ...
    Categoria.findByIdAndRemove({ _id: id }, (err, categoria_delete) => {
        if (err) {
            ...
            if (categoria_delete) {
                ...
                res.status(403).send({ message: 'La categoria no pudo ser eliminada' });
            }
        }
    ...
    Categoria.find({ titulo: new RegExp(nombre, 'i') }, (err, categoria_listado) => {
        ...
        if (err) {
            ...
            if (categoria_listado) {
                ...
                res.status(403).sen({ message: 'No hay registro de la categoria ingresada' });
            }
        }
