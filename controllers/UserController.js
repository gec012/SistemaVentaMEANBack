...
function editar(req, res) {
    ...
    User.findById(id, (err, user_data) => {
        if (user_data) {
            ...
            if (user_data.password != data.password && data.password != "") {
                console.log('cambio cntraseña');
                bcrypt.hash(data.password, null, null, function(err, hash) {
                    if (hash) {
                        User.findByIdAndUpdate(id, { nombre: data.nombre, apellido: data.apellido, password: hash, email: data.email, role: data.role }, (err, user_edit) => {
                            if (user_edit) {
                ...
                console.log('no cambio cntraseña');
                User.findByIdAndUpdate(id, { nombre: data.nombre, apellido: data.apellido, email: data.email, role: data.role }, (err, user_edit) => {
                    if (user_edit) {
                        res.status(200).send({ user: user_edit });
                    } else {
                        res.status(500).send({ message: 'El usuario no se pudo editar' });
