    ...
    Producto.find({ titulo: new RegExp(titulo, 'i') }).populate('idcategoria').exec((err, producto_listado) => {
        if (err) {
            ...
            if (producto_listado) {
                ...
                res.status(403).send({ message: 'Error en el servidor' });
            }
        }
...
function editar(req, res) {
    ...
    if (req.files.imagen) {
        ...
        Producto.findByIdAndUpdate({ _id: id }, { titulo: data.titulo, descripcion: data.descripcion, stock: data.stock, precio_compra: data.precio_compra, precio_venta: data.precio_venta, idcategoria: data.idcategoria, puntos: data.puntos }, (err, producto_edit) => {
            if (err) {
                res.status(500).send({ message: 'Error en el servidor' });
            } else {
                if (producto_edit) {
